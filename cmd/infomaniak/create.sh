...
KEYPAIR="${INFOMANIAK_KEYPAIR:-devbox}"
SG="${INFOMANIAK_SG:-devbox}"
...
if id="$(server_id_by_name "$NAME")" && [[ -n "$id" ]]; then
  ...
  user_data="$(mktemp -t devbox-infomaniak-userdata.XXXXXX)"
...
# Wait for SSH as root. Use DEVBOX_SSH_KEY (a passphrase-less key) when set so
