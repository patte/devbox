...
#   PUBKEY_FILE  default ${DEVBOX_SSH_KEY}.pub, else ~/.ssh/id_ed25519.pub
#
...
# an Exoscale SSH key (lands on the default user too). A dedicated `devbox`
...
if id="$(instance_id_by_name "$NAME")" && [[ -n "$id" ]]; then
  ...
  # Ensure the `devbox` security group (ssh + tailscale + icmp from anywhere).
  ensure_sg "$SG" >/dev/null

  # cloud-init: put the key on root and re-enable root SSH login.
  user_data="$(mktemp -t devbox-exoscale-userdata.XXXXXX)"
...
# Wait for SSH as root. Use DEVBOX_SSH_KEY (a passphrase-less key) when set so the
